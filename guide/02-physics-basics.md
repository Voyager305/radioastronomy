...
Встречающиеся математические знаки:

| Знак | Читается | Что означает |
|------|----------|--------------|
| ≈ | приближённо равно | значение округлено или неточно |
| ∼ | порядка | грубая оценка величины (например, «∼ 1 МГц») |
| ∝ | пропорционально | одна величина растёт вслед за другой в постоянном отношении |

Подробнее про **∝**: запись `A ∝ B` означает `A = k · B` с некоторым постоянным коэффициентом `k` (его конкретное значение не важно). Знак показывает не абсолютную величину, а **характер связи**. Например, `SNR ∝ √τ`: увеличение времени накопления τ в 4 раза увеличивает √τ в 2 раза, а значит и SNR — примерно в 2 раза.
